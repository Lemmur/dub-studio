...
| `libsndfile` / `SoXR` / `Eigen` | — | `LGPL`/`MIT` | линковка ядра — перед подключением проверить каждую | запланировано |
| `WSOLA time-stretch` | собственный код ([`core/audio/audio_ops.cpp`](../core/audio/audio_ops.cpp)) | собственная (`MIT`, проект) | `timeStretchWsola` — растяжение с сохранением питча для `Fit to Ref` | подключено (Фаза 2) |

Примечания:

- `RubberBand` — решение (Фаза 2, зафиксировано): библиотека `GPL` + коммерческая,
  в ядро НЕ линкуется (правило AGENTS.md «без GPL-заражения»). Вместо неё написан
  собственный `WSOLA` (окно Ханна, overlap 50%, нормированная кросс-корреляция) —
  для моно-речи 0.25x–4.0x достаточно. При потребности в качестве выше — RubberBand
  CLI как внешний `subprocess` (по образцу vgmstream, Фаза 3+).
